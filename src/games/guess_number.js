...
function guessNumberGame(secretNumber, guesses, range) {
  ...
  for (let guess = 0; guess < guesses; guess++) {
    ...
    const message = response > secretNumber
      ? `Too High ${emoji}`
      : `Too Low ${emoji}`;
    console.log(message);
  }
...
function startGame(level) {
  switch (level) {
    case "1":
      return guessNumberGame(generateSecretNumber(50), 5, "1-50");
    case "2":
      return guessNumberGame(generateSecretNumber(100), 5, "1-100");
    case "3":
      return guessNumberGame(generateSecretNumber(500), 6, "1-500");
    case "4":
      return guessNumberGame(generateSecretNumber(1000), 8, "1-1000");
    default: {
